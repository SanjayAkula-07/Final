...
# =====================================================================
# 1. ENTERPRISE DATABASES & APIs (Starts empty)
# =====================================================================
...
# --- NEW FEATURE: FORECASTING TOOL ---
def forecast_budget(user_id: str = "U102") -> str:
    """Forecasts if the user will stay within budget by the end of the month based on current spending pace."""
    if user_id not in USER_DB:
    ...
    if budget <= 0:
        return json.dumps({"error": "Budget is not set. Please set a budget first."})
    
    if total_spent <= 0:
        return json.dumps({"status": "No expenses logged yet. Spend some money before I can forecast!"})

    # Get current day of the month and total days in the month
    ...
    if projected_spend > budget:
        status = "Projected Overrun"
        shortfall = projected_spend - budget
        message = f"Warning: At a pace of ₹{daily_rate:.0f}/day, you will overspend your budget by ₹{shortfall:.0f}."
    else:
        status = "On Track"
        savings = budget - projected_spend
        message = f"Good job! At a pace of ₹{daily_rate:.0f}/day, you will end the month with ₹{savings:.0f} left."
        
    return json.dumps({
        "current_day": current_day,
        "days_in_month": days_in_month,
        "daily_burn_rate_inr": round(daily_rate, 2),
        "projected_end_of_month_spend": round(projected_spend, 2),
        "status": status,
        "ai_alert_message": message
    })

# Add the new tool to the list
financial_tools = [get_financial_profile, log_expense, fetch_market_data, calculate_confidence_score, forecast_budget]
...
try:
    ...
    chat_config = types.GenerateContentConfig(
        ...
        system_instruction=(
            ...
            "4. If asked about forecasting, predicting, or if they will have enough money left by the end of the month, use forecast_budget. "
            "Always present the final answer clearly. Write for an everyday person, not a finance professional: start with a one-line answer (yes / no / not yet), "
...
def render_dashboard(user_id: str = "U102") -> str:
    ...
    goals_html = "".join(goals) or "<p class='empty'>No savings goals yet. Add one in settings.</p>"
...
def save_profile(budget, cash, goal_name, goal_amt):
    try:
        b = float(budget or 0)
        c = float(cash or 0)
        g = float(goal_amt or 0)
    except (TypeError, ValueError):
        b, c, g = 0.0, 0.0, 0.0

    USER_DB["U102"]["monthly_budget"] = b
    USER_DB["U102"]["liquid_cash"] = c
    
    if goal_name and goal_name.strip() and g > 0:
        USER_DB["U102"]["savings_goals"][goal_name.strip()] = g

    return render_dashboard(), _debug_state(), "<div class='add-status ok'>Profile saved successfully!</div>"
...
def bot_respond(history):
    ...
    if not API_KEY or API_KEY == "YOUR_GEMINI_API_KEY_HERE" or chat is None:
        history = history + [{
            "role": "assistant",
            "content": "⚠️ **Configuration Error:** The AI agent failed to start. Please check your `GEMINI_API_KEY` and ensure it is valid."
        }]
    ...
    try:
        ...
        reply_text = f"**⚠️ Something went wrong while calling the AI.**\n\nError details: `{error_msg}`\n\nPlease check your API key or connection and try again."

    history = history[:-1] + [{"role": "assistant", "content": reply_text}]
    yield history, render_dashboard(), _debug_state()

# --- Voice Transcription Function ---
def transcribe_audio(audio_path):
    """Takes microphone audio, uploads it to Gemini, and returns the transcribed text."""
    if not audio_path:
        return ""
    if client is None:
        return "⚠️ API Key missing. Cannot transcribe."
    
    try:
        audio_file = client.files.upload(file=audio_path)
        response = client.models.generate_content(
            model=MODEL_NAME,
            contents=[audio_file, "Please transcribe this audio exactly as spoken. Return only the text."]
        )
...
# ADDED NEW PROMPT FOR FORECASTING
QUICK_PROMPTS = [
    ("Log ₹500 for fuel", "I just spent ₹500 on fuel, please log it."),
    ("Forecast budget", "Based on my spending so far, will I stay within my budget this month?"),
...
CHAT_PLACEHOLDER = (
    ...
    "I can log an expense, check your budget, forecast your monthly spend, or tell you whether a purchase makes sense. "
    "Tap a suggestion below, or use the microphone to speak."
)
...
with gr.Blocks(title="Money Assistant", **blocks_options) as demo:
    ...
    with gr.Row(equal_height=False):
        ...
        with gr.Column(scale=5, min_width=320):
            ...
            with gr.Accordion("⚙️ Setup Profile & Goals", open=True):
                gr.Markdown("Set your starting numbers before logging expenses.")
                with gr.Row():
                    set_budget = gr.Number(label="Monthly Budget (₹)", value=0, min_width=120)
                    set_cash = gr.Number(label="Available Cash (₹)", value=0, min_width=120)
                with gr.Row():
                    set_goal_name = gr.Textbox(label="Savings Goal Name", placeholder="e.g. Goa Trip", min_width=120)
                    set_goal_amt = gr.Number(label="Goal Target (₹)", value=0, min_width=120)
                
                save_prof_btn = gr.Button("Save Profile Settings", variant="secondary")
                save_status = gr.HTML("")
    ...
    save_prof_btn.click(
        save_profile,
        [set_budget, set_cash, set_goal_name, set_goal_amt],
        [dashboard_html, debug_json, save_status]
    )
    ...
    # Voice Input Wiring: Transcribe audio and paste into textbox
    mic_input.stop_recording(
        transcribe_audio, 
        inputs=[mic_input], 
        outputs=[txt]
    ).then(
        lambda: None, None, mic_input # Clears the microphone widget after transcribing
    )
